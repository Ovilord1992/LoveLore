...
- Загрузка ZIP: кнопка «Загрузить ZIP» → выбор файла → автоматическое извлечение meta.json + обложки + создание глав
- Удаление новеллы (удаляет ZIP, обложку и запись из БД)
- Информация: название, автор, главы (released/total), загрузки, размер файла, версия

### Управление главами (API)

- `GET /admin/novels/:id/chapters` — список глав с флагами isReleased
- `PATCH /admin/novels/:id/chapters/:number` — выпустить или скрыть главу, назначить дату выхода
- Модель «живого контента»: можно выпускать главы по расписанию

> **Примечание:** фронтенд-страница управления главами в разработке. Пока управление через API.
...
| `GET /admin/novels/:id/chapters` | Список глав новеллы |
| `PATCH /admin/novels/:id/chapters/:number` | Выпуск/скрытие главы |
| `POST /novels/upload` | Загрузка ZIP (авто-создание глав) |
